...
using CashFlow.Entries.Infra.Data.Helpers;
...
namespace CashFlow.Entries.Infra.Data.Gateways
{
    public class GenericRepository<T>(IDatabaseContext context) : IGenericRepository<T> where T : class
    {
        ...
        public async Task<ICollection<TOutput>> WhereAsync<TOutput>(Expression<Func<T, bool>> query, Expression<Func<T, TOutput>> selector, CancellationToken cancellationToken = default)
        {
            var collection = await _queryable
                .Where(query)
                .Select(selector)
        ...
        public async Task<long> SumAsync(Expression<Func<T, bool>> query, Expression<Func<T, long>> selector, CancellationToken cancellationToken = default)
        {
            return await _queryable
                 .Where(query)
